...
int main() {
  ...
  httpsServer.resource["^/$"]["GET"] = [](shared_ptr<HttpsServer::Response> response, shared_ptr<HttpsServer::Request> request) {
    try {
  ...
  httpsServer.resource["^/jquery-3.2.1.min.js$"]["GET"] = [](shared_ptr<HttpsServer::Response> response, shared_ptr<HttpsServer::Request> request) {
	  try {
		  string path = "jquery-3.2.1.min.js";
		  SimpleWeb::CaseInsensitiveMultimap header;
		  auto ifs = make_shared<ifstream>();
		  ifs->open(path, ifstream::in | ios::binary | ios::ate);
